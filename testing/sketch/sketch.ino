...
Adafruit_NeoPixel strip2 = Adafruit_NeoPixel(60, PIN_2, NEO_GRB + NEO_KHZ800);
...
void turnPixelsOff(uint16_t wait) {
...
// Fill the dots one after the other with a color
void strip2_colorWipe(uint32_t c, uint8_t wait) {
  Serial.println(c);
...
void strip2_rainbow(uint8_t wait) {
  uint16_t i, j;

  for(j=0; j<256; j++) {
    for(i=0; i<strip2.numPixels(); i++) {
      strip2.setPixelColor(i, strip2_Wheel((i+j) & 255));
    }
    strip2.show();
    delay(wait);
  }
...
// Slightly different, this makes the rainbow equally distributed throughout
void strip2_rainbowCycle(uint8_t wait) {
  uint16_t i, j;

  for(j=0; j<256*5; j++) { // 5 cycles of all colors on wheel
    for(i=0; i< strip2.numPixels(); i++) {
      strip2.setPixelColor(i, strip2_Wheel(((i * 256 / strip2.numPixels()) + j) & 255));
    }
    strip2.show();
    delay(wait);
  }
...
//Theatre-style crawling lights.
void strip2_theaterChase(uint32_t c, uint8_t wait) {
  for (int j=0; j<10; j++) {  //do 10 cycles of chasing
    for (int q=0; q < 3; q++) {
...
//Theatre-style crawling lights with rainbow effect
void strip2_theaterChaseRainbow(uint8_t wait) {
  for (int j=0; j < 256; j++) {     // cycle all 256 colors in the wheel
    for (int q=0; q < 3; q++) {
      for (uint16_t i=0; i < strip2.numPixels(); i=i+3) {
        strip2.setPixelColor(i+q, strip2_Wheel( (i+j) % 255));    //turn every third pixel on
      }
      strip2.show();
...
// Input a value 0 to 255 to get a color value.
// The colours are a transition r - g - b - back to r.
uint32_t strip2_Wheel(byte WheelPos) {
  WheelPos = 255 - WheelPos;
  if(WheelPos < 85) {
